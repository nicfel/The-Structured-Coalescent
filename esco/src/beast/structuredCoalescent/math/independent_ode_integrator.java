...
public class independent_ode_integrator implements FirstOrderDifferentialEquations {
    ...
    // constructor
    public independent_ode_integrator(double[][] migration_rates, double[] coalescent_rates, int lineages , int states){
    	...
    	for (int i = 0; i<lineages; i++){
    		...
    		for (int j = 0; j<states; j++){
    			...
    			pDot[states*i+j] = migrates -
    					  coalescent_rates[j] * p[states*i+j] * (sumStates[j] - p_norm[states*i+j]);
    		}// j
    	}// lineages    	
    }
