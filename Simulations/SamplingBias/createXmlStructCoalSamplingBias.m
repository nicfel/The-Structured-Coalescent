...
%     tree_tmp1 = strrep(tree_tmp1,'kickout]','');
    ...
    for tr = 1 : 3    
        ...
        flog = strrep(tree_files(i).name,'master.tree',sprintf('%desco',tr));
        ...
        migini = lognrnd(log(migrate),0.4,1,2);
        ...
        fprintf(g,'\t\t\t\t<distribution id="coalescent" spec="ExactStructuredCoalescent" dim="2">\n');
        ...
        fprintf(g,'\t\t<logger id="tracelog" fileName="%s.log" logEvery="100" model="@posterior" sanitiseHeaders="true" sort="smart">\n',flog);
...
for i = 1 : length(iscoxmls)
    ...
    g = fopen(sprintf('xmls/%s',strrep(iscoxmls(i).name,'esco','lisco')),'w');
    while ~feof(f)
        line = fgets(f);
        if ~isempty(strfind(line,'chainLength="100000"'))
            fprintf(g, '%s', strrep(line,'chainLength="100000"','chainLength="200000"'));            
...
for i = 1 : length(iscoxmls)
    f = fopen(sprintf('xmls/%s',iscoxmls(i).name),'r');
    g = fopen(sprintf('xmls/%s',strrep(iscoxmls(i).name,'esco','sisco')),'w');
    while ~feof(f)
        line = fgets(f);
        if ~isempty(strfind(line,'chainLength="100000"'))
            fprintf(g, '%s', strrep(line,'chainLength="100000"','chainLength="300000"'));
        elseif ~isempty(strfind(line,'ExactStructuredCoalescent'))
            fprintf(g, '%s', strrep(line,'ExactStructuredCoalescent"','ApproximateStructuredCoalescent"'));
        elseif ~isempty(strfind(line,'esco.log'))
            fprintf(g, '%s', strrep(strrep(line,'esco.log','sisco.log'),'logEvery="100"','logEvery="300"'));
        elseif ~isempty(strfind(line,'name="timeStep">0.001'))
            fprintf(g, '%s', strrep(line,'name="timeStep">0.001','name="timeStep">0.01'));
        else
            fprintf(g, '%s', line);
