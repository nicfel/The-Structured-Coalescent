...
# coalescent for a 10 tip tree sampled homochronously 
...
for (i in 1:length(log_files)){
  ...
  t_lisco <- read.table(gsub("esco","lisco",filename), header=TRUE, sep="\t")
  ...
  m_lisco <- which.max(t_lisco$posterior)
  ...
  # was too small
    ...
    print(m_lisco)
    print(length(t_lisco$posterior))
    print(m_sisco)
    print(filename)
  }
}
...
p <- ggplot(data=rate)+
  geom_point(aes(color="ESCO",x=rate,y=esco),size=0.5) +
  geom_point(aes(color="LISCO",x=rate,y=lisco),size=0.5) +
  geom_point(aes(color="SISCO",x=rate,y=sisco),size=0.5) +
  xlab("true migration rate") + ylab("estimated migration rate")  +
  scale_colour_manual("",values = c("ESCO" = col3, "LISCO" = col4, "SISCO" = col2)) +
  scale_x_log10() + scale_y_log10()  + guides(colour = guide_legend(override.aes = list(size=2)))

d <- data.frame(x=c(0.00001,1), y=c(0.00001,1))
p <- p + geom_line(data=d,aes(x=x, y=y), color="red", linetype="dashed")
plot(p)

ggsave(plot=p,"../../text/figures/migrationRates/migrationRateEstimates.eps",width=7, height=5)
